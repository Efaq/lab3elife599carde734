  ...
  
{
  ...
  if((is.data.frame(graph)== "FALSE" || length(graph)!=3)  ||( (is.numeric(init_node)=="FALSE") || length(init_node)>1 || init_node %in% graph[[1]] ==FALSE ))
  {
  ...
  while(length(unvisited)!=0)
  {
    if(!is.null(visited)) 
    {
      ...
      {
        index<-which(shortest_dist[]==min(shortest_dist[(setdiff(vertex, visited))]))
      }
    ...
    if(!is.null(visited)) 
    {
      index_use<- which(destination_vertex %in% setdiff(destination_vertex, visited) )
    ...
    { 
      ...
      if (length(destination_vertex)==0) break;
      ...
      {
        shortest_dist[destination_vertex[i]]<-weight_vertex[i]+shortest_dist[element_min]
